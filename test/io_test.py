...
def test_open_right_object(typeexamples_nmea_path):
    """Make sure ais_crc.open() returns the right object"""
    with ais_crc.open(typeexamples_nmea_path) as src:
        assert isinstance(src, ais_crc.NmeaFile)
    # pytest places a different object in sys.stdin so do an old-shool
    # type comparison
    with ais_crc.open('-') as src:
        assert isinstance(src._fobj, type(sys.stdin))


def test_open_path(typeexamples_nmea_path):
    """ais_crc.open() with a file path on disk"""
    with ais_crc.open(typeexamples_nmea_path) as src:
...
def test_open_file_like_object(bare_nmea):
    """Make sure ais_crc.open() can handle a file-like object."""
    with contextlib.closing(StringIO(bare_nmea)) as f, ais_crc.open(f) as src:
        for idx, line in enumerate(src):
            assert isinstance(line, dict)
            print(line)
        assert idx >= 6
...
def test_open_exceptions():
    ...
    with pytest.raises(ValueError):
        ais_crc.open('something', mode='bad-mode')
